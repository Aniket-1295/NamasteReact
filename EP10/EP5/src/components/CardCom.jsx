    ...
    return (
        ...
        <div className="bg-white shadow-lg rounded-lg p-4 hover:shadow-xl transition-shadow duration-300 flex flex-col items-center text-center">
            <img 
                ...
                alt="cardImg" 
            />
            <h3 className="cardHead text-lg font-semibold text-gray-800 mb-2">{resData?.info?.name}</h3>
            <h4 className="cardCusine text-sm text-gray-600 mb-2">{resData?.info?.cuisines?.join(", ")}</h4>
            <h4 className="cardRating text-sm text-yellow-500 font-medium mb-2">{resData?.info?.avgRating}⭐</h4>
            <h4 className="cardPrice text-sm text-gray-700">{resData?.info?.costForTwo}</h4>
        </div>
        </>
    )
}
